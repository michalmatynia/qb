...
export function ColumnRow({ column, item, formcellkey, formcell, sublistkey, tiedtoformkey, handleSwitch, toggleItem, changePosition, changeQuantity, removeItem, model = null }) {
    ...
    if (column.columntype === 'image') {
        ...
        return <IconButton
            {...column.inputprops}
            onClick={(event) => column.actions.onClick({ event, value: item, cell, sublistkey, tiedtoformkey })}
            key={item._id}
        >
