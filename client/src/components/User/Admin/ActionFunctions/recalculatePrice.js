...
export async function actionFuncs_recalculatePrice_v3({ current_products, dispatch, current_mysite = null, currencyuser = null, inQuery = {}}) {

    let current_lgbinders = current_products.payload.map(a => a.lgbinder)

    Object.assign(inQuery, {
        country: { "$eq": current_mysite.default_language.referenceID.alpha2Code },
        language: { "$eq": current_mysite.default_language.referenceID.languages[0].iso639_1 },
        lgbinder: { "$in": current_lgbinders }
    });
    ...
    let recalculated_list = current_products.payload.map((item) => {
