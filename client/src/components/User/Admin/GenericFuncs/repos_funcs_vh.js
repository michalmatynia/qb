...
export async function reposFuncs_soloPositionSwap_vh2({ model, dispatch = null, current = null, updated = null, poliglot = null }) {
...
export async function reposFuncs_syncPositionAcross_vh1({ model, dispatch,  updated = null, redux_module, country = null, language = null }) {

    let inQuery
    let inOperator
    let inParams

    if (updated.payload.position !== redux_module.detail.position || updated.payload.visible !== redux_module.detail.visible) {
        inQuery = {
            country: { "$eq": country },
        ...
        let lgbupdated = await plg_updateOne_queMod_oprMod({ model, dispatch, actionType: 'samestate', inQuery, inOperator, inParams })
...
export async function reposFuncs_ChangePosition_vh1({ item = null, direction = null, model = null, dispatch = null, poliglot = null, redux_localeuser = null }) {
    let inQuery
    let inOperator
    let inParams
    ...
    let allitems = await plg_countDocuments({ model, dispatch, actionType: 'samestate', inQuery })
    ...
    let target = await plg_findOne_QueMod({ model, actionType: 'samestate', dispatch, inQuery })
    ...
    if (
        ...
        targetresponse = await plg_updateOne_queMod_oprMod({ model, actionType: 'samestate', dispatch, inQuery, inOperator, inParams })
        ...
        endgameresponse = await plg_updateOne_queMod_oprMod({ model, actionType: 'samestate', dispatch, inQuery, inOperator })
        ...
        if (poliglot) {
            ...
            if (target.payload.lgbinder) {
                ...
                let otherlanguages = await plg_findMany({  model, dispatch, actionType: 'samestate', inQuery })
                ...
                for (const value of Object.values(otherlanguages.payload)) {
                    ...
                    countwithlgbinder = await plg_countDocuments({ model, dispatch, actionType: 'samestate', inQuery })
                    ...
                    if (
                        ...
                        if ('lgbinder' in endgameresponse.payload && endgameresponse.payload.lgbinder !== '') {
                            ...
                            samepositem = await plg_findOne_QueMod({ model, dispatch, actionType: 'samestate', inQuery })
                        ...
                        let lgbtargetresponse = await plg_updateOne_queMod_oprMod({ model, actionType: 'samestate', dispatch, inQuery, inOperator, inParams })
...
export async function reposFuncs_UpdatePosition_vh1({ country = null, language = null, removed = null, model = null, dispatch = null, poliglot = null }) {
    let inQuery
    let inOperator
...
export async function reposFuncs_whenAdded_vh1({ dispatch = null, country = null, language = null, added = null, model = null, poliglot = null }) {
    let inQuery
    let inOperator
