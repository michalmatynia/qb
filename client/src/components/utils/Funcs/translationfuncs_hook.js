...
export async function runtranslation_vh2({ source_lg = null, target_lg = null, content = null,  engine = null, dispatch }) {
    let translation
    let response

    try {

        let axiosconfig = await makeaxiosconf({ engine, source_lg, target_lg, content })
        let updatedengine = await updateEngine({engine, dispatch})
...
export async function updateEngine({ engine = null, dispatch = null }) {
    ...
    let result = await plg_updateOne_queMod_oprMod({ model: 'transengine', dispatch, actionType: 'samestate', inQuery, inOperator, inParams })

    return result
    // return engine
}
const makeaxiosconf = async ({ engine = null, source_lg = null, target_lg = null, content = null }) => {
